...
func schemaOne(db *sql.DB) {
	...
	ExecQuery(db, "CREATE TABLE IF NOT EXISTS public.files (id uuid DEFAULT uuid_generate_v4 (), coverImage varchar(255), source varchar(255), url varchar(255), version numeric, created_at TIMESTAMPTZ, PRIMARY KEY (id));")
	...
	ExecQuery(db, "CREATE TABLE IF NOT EXISTS public.users (id uuid DEFAULT uuid_generate_v4 (), name varchar(255), email varchar(255), password varchar(255), created_at TIMESTAMPTZ, PRIMARY KEY (id));")
